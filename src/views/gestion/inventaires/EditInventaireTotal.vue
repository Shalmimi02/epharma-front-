<template>
    ...
    <div v-else>
        ...
        <div class="d-flex border p-3 mb-3 bg-light">
            ...
            <button v-if="inventaire.statut == 'En cours' && hasPermission('PRIVILEGES', 'inventaire_master')" type="button" class="btn btn-light mr-2"
                @click="updateInventaire('Terminé')">
                <i class="fa-solid fa-check"></i> Terminer
            </button>
            <button v-if="inventaire.statut == 'En cours' && hasPermission('PRIVILEGES', 'inventaire_master')" class="btn btn-danger ml-auto"
                @click="updateInventaire('Annulé')">
        ...
        <div v-for="(rayon, index) in rayons" :key="index" class="mb-3">
            ...
            <SectionVisibility :libelleSection="'rayon ' + rayon" :isOpen="true">
