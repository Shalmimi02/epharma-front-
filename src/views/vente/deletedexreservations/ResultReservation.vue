        ...
        <div class=" p-1">
            <form @submit.prevent="updateReservation" class="mb-2">
                <div class="form-group">
                    <label>Remise:</label>
                    <input type="number" class="form-control border border-dark mb-3" @input="calculRemise" v-model="formRemise.remise" required :disabled="isTreatable() == false ">
                </div>
                <button type="submit" class="btn btn-info btn-block" v-if="isTreatable() == true && formRemise.remise > 0 && selectedReservation.remise != formRemise.remise">
                    Appliquer
                    <span v-if="remiseIsLoading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                </button>
            </form>
        </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        updateReservation() {
            ...
            this.axios.post(`${this.baseUrl}/api/reservations/${this.selectedReservation.id}/update`, form).then(response => {
