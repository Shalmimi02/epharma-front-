...
export default defineComponent({
    ...
    methods: {
        ...
        initSelection() {
            this.isLoading = true
            this.axios.get( '/api/' + this.table + '?reservation_id=' + this.reservationId).then(response => {
        ...
        updateReservationProduit(line) {
            ...
            this.axios.post('/api/' + this.table, form).then(response => {
                this.isLoading = false
                if (response.data.success === true) {
        ...
        removeReservationProduit(produitId) {
            ...
            this.axios.post('/api/' + this.table + '/' + produitId + '/destroy2', form).then(response => {
                this.isLoading = false
                if (response.data.success === true) {
                    $('#refresh' + this.table).click()
                    this.$emit('refreshReservation')
